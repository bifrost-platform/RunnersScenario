...
Chainrunner Q는 이 문서의 내용대로 BiFi에 KDAI를 예금합니다.

### 예금할 금액 설정하기

- 예금할 금액을 설정합니다. (보유하신 KDAI가 부족하거나 잘못된 숫자가 입력된 경우에는 에러가 발생합니다.)

```input KDAI
// 예금할 KDAI 수량 (소수점도 가능합니다.)
...
assert(Q.Token.balanceOf("kdai") >= amountDeposit, "보유한 KDAI가 부족합니다.");
```
