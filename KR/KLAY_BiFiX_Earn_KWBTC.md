...
Chainrunner Q는 이 문서의 내용대로, KWBTC 토큰을 가지고 BiFi-X Earn 서비스를 간편하게 사용할 수 있도록 합니다.

### 사용할 KWBTC를 입력 받습니다.

- 지갑에 보유중인 KWBTC를 확인하고, 서비스에 사용할 수량을 입력합니다.
- 만약, KWBTC를 보유하고 있지 않다면, **토큰 구매하기** 메뉴에서 구매 후 이용하시기 바랍니다.
...
assert(amount <= Q.Token.balanceOf ("kwbtc"), "KWBTC 잔액이 부족합니다.");
```

### BiFi-X에 KWBTC를 최대 배율로 레버리지하고 Earn을 합니다.

```taster
// BiFi-X에서 레버리지 할 수 있는 KWBTC의 최대 배율을 확인합니다
let maxBoost = Q.bifiX.getMaxBoost ("kwbtc");

// 포지션을 생성하기 앞서, KWBTC 토큰을 승인합니다.
